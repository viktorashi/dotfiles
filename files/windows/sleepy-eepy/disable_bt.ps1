# Kill any pending or running enable script so Sleep wins
Get-CimInstance Win32_Process -Filter "CommandLine LIKE '%enable_bt.ps1%'" -ErrorAction SilentlyContinue |
...
if ($bt) {
    $setOp = $bt.SetStateAsync([Windows.Devices.Radios.RadioState]::Off)
    ...
    Write-Host "Bluetooth Radio turned Off (Result: $($tSet.Result))"
} else {

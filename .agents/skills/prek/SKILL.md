...
## Patching generated hook shims

`prek install` generates hook shims in `.git/hooks/`. These are **not tracked by git**. Any customisation (e.g. conditional `-q` flags) lives only in `.git/` and is lost on `prek install -f`.

**Rule**: whenever you patch a generated hook shim, also create a tracked `install-hooks.sh` script in the repo (or relevant subproject dir) that re-applies the patch idempotently. Point users to run it after `prek install -f`.

### Subproject context-aware quieting

When a prek.toml lives in a subdirectory with `orphan = true`, the installed hook runs for every commit in the whole repo and shows "Skipped" on unrelated commits. To suppress that noise only when committing from outside the subdir, patch the exec line in the hook:
...
case "$PWD" in
  */your-subdir*)
    exec "$PREK" hook-impl ... ;;
  *)
    exec "$PREK" -q hook-impl ... ;;
esac
```

`prek install -f -q` bakes `-q` globally into the shim (use when you always want quiet). For the conditional approach above you must patch after install — hence the `install-hooks.sh` script.

The `sed` pattern to replace the bare exec line idempotently:

```sh
sed -i "s|exec \"\$PREK\" hook-impl \(.*--hook-type=TYPE.*\)|case \"\$PWD\" in\n  */your-subdir*)\n    exec \"\$PREK\" hook-impl \1 ;;\n  *)\n    exec \"\$PREK\" -q hook-impl \1 ;;\nesac|" "$hook_file"
```

### Preferred: write hooks directly from install-hooks.sh

Rather than sed-patching, write the full hook file as a heredoc. This is more robust and lets you use richer logic — for example, checking **staged files** (pre-commit) or **pushed commits** (pre-push) instead of `$PWD`, so the hook shows full output (including the progress bar) only when relevant files are actually being committed/pushed:

**pre-commit** — check staged files:
```sh
if git diff --cached --name-only | grep -q "^your-subdir/"; then
    exec "$PREK" hook-impl ... "$@"
else
    exec "$PREK" -q hook-impl ... "$@"
fi
...
**pre-push** — read refs from stdin, check pushed commits, replay stdin:
```sh
input=$(cat)
touches=false
while IFS=' ' read -r _lref lsha _rref rsha; do
    ...
    range="${rsha}..${lsha}"
    [ "$rsha" = "0000000000000000000000000000000000000000" ] && range="$lsha"
    if git diff --name-only "$range" 2>/dev/null | grep -q "^your-subdir/"; then
        touches=true; break
    fi
done << EOF
$input
...
if [ "$touches" = true ]; then
    printf '%s\n' "$input" | "$PREK" hook-impl ... "$@"
else
    printf '%s\n' "$input" | "$PREK" -q hook-impl ... "$@"
fi
exit $?
```

Note: pre-push stdin must be buffered with `input=$(cat)` and replayed with `printf '%s\n' "$input" |` because prek itself reads from stdin; `exec` cannot be used with a pipe so capture the exit code with `exit $?`.
